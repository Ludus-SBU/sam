...
cp burn $work_dir
cp track_puller.py $work_dir
cp make_initial_abundance_MESAhybrid.py $work_dir

cd $work_dir
python3 track_puller.py ../$file $n > trho_hist.dat
./burn || { echo "particle $n: burn failed" >> ../failed_particles.log; 
    cd ../;
    rm -r $work_dir;
    exit 1; }
mv final_abundances.dat ../final_abundances_2D/final_abundances_$n.dat
